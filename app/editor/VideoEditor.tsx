  ...
  const handleTimelineDrop = useCallback((e: React.DragEvent) => {
    ...
    try {
      ...
      if (data.type === 'asset' && data.assetId) {
        ...
        const dropX = e.clientX - timelineRect.left - 192; // Account for track header
        const dropTime = Math.max(0, dropX / snapshot.timelineZoom);
        
        // Find target track (basic implementation - drop on first track)
        const targetTrack = snapshot.tracks[0];
