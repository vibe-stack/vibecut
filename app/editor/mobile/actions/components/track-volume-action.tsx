...
export const TrackVolumeAction: React.FC<{ trackId: string; volume: number }> = ({ trackId, volume }) => {
          ...
          <Drawer.Content className="fixed bottom-0 left-0 right-0 rounded-t-2xl bg-neutral-900 text-white p-4">
            <div className="mx-auto max-w-md">
              ...
              <input
                ...
                className="w-full"
                value={volume}
                onChange={(e) => editorActions.updateTrack(trackId, { volume: parseFloat(e.target.value) })}
              />
            </div>
          </Drawer.Content>
        </Drawer.Portal>
...
export default TrackVolumeAction;
