...
export const AudioSpeedAction: React.FC<{ clipId: string }> = ({ clipId }) => {
  ...
  const setValue = (v: number) => editorActions.setAudioEffects(clipId, { speed: Math.max(0.25, Math.min(4, v)) });
  const reset = () => editorActions.setAudioEffects(clipId, { speed: 1 });
...
export default AudioSpeedAction;
