...
export const AudioFadeInAction: React.FC<{ clipId: string }> = ({ clipId }) => {
  ...
  const setValue = (v: number) => editorActions.setAudioEffects(clipId, { fadeInSec: Math.max(0, Math.min(clip.duration, v)) });
  const reset = () => editorActions.setAudioEffects(clipId, { fadeInSec: 0 });

  return (
    <>
      <ActionButton onClick={() => setOpen(true)}>
        <Volume1 size={18} />
        <span className="text-[10px] opacity-80">Fade In</span>
      </ActionButton>
          ...
          <Drawer.Content className="fixed bottom-0 left-0 right-0 rounded-t-2xl bg-neutral-900 text-white p-4">
            <div className="mx-auto max-w-md">
              ...
              <div className="grid grid-cols-[80px_1fr_48px] items-center gap-2">
                ...
                />
                <div className="text-[10px] text-white/60 tabular-nums text-right">{effects.fadeInSec.toFixed(1)}s</div>
              </div>
            </div>
          </Drawer.Content>
        </Drawer.Portal>
...
export default AudioFadeInAction;
